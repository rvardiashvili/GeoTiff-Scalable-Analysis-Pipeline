...
\begin{enumerate}
    ...
    \item \textbf{ResNet-50 (All Bands):} A standard \CNNs{} baseline utilizing all available Sentinel-2 and Sentinel-1 bands for multi-modal fusion.
    \item \textbf{ResNet-50 (S2):} A variant optimized for Sentinel-2 spectral characteristics.
    \item \textbf{ConvNeXt-S2:} A modern \CNNs{} architecture optimized for satellite imagery.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Comparison of Inference Throughput across Models}
...
\begin{figure}[H]
    ...
    \caption{Memory Footprint Analysis: VRAM vs System RAM}
    \label{fig:memory}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/gpu_stats.png}
    \caption{GPU Efficiency: Utilization and Thermal Stress}
    \label{fig:gpu_stats}
\end{figure}
...
\begin{itemize}
    \item \textbf{Peak Temperatures:} All models pushed the mobile GPU to its thermal limit (\textbf{88-90$^\circ$C}). This is typical for laptop GPUs but highlights the importance of the \textit{efficiency} improvements (e.g., faster inference = less time at max temp) for hardware longevity.
    \item \textbf{ConvNeXt Stress:} The extended 636s runtime of ConvNeXt at 90$^\circ$C likely triggered thermal throttling, which may have further reduced its throughput compared to the shorter bursts of ResNet.
\end{itemize}
...
The pipeline successfully generated multi-layered geospatial products for all tested models. Qualitatively, even a simple visual inspection demonstrates the effectiveness of the ResNet-All model compared to rudimentary baselines like simple pixel-wise thresholding or majority voting, which often struggle with complex land cover patterns. Figure \ref{fig:qualitative} illustrates the Sentinel-2 input image and the corresponding outputs of the ResNet-All model. The input image (top-left) provides a true-color view of the area. The Class Map (top-right) clearly delineates different land cover classes, while the Entropy Map (bottom-left) highlights areas of model uncertainty, typically along class boundaries where mixed pixels occur. The Prediction Gap map (bottom-right) provides a complementary measure of confidence margin.


\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/qualitative.png}
    \caption{Qualitative Outputs: Sentinel-2 input image, Class Map, Entropy (Uncertainty), and Prediction Gap for the ResNet-All model.}
...
\begin{itemize}
    ...
    \item \textbf{Feasibility:} This demonstrates that continental-scale analysis is feasible even without a supercomputer. A single laptop could process ~140 tiles per day (using ResNet), covering a small country in under a week.
\end{itemize}
